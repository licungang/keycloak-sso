...
= Dark mode enabled for all `keycloak` themes

We've now enabled dark mode support for all the `keycloak` themes. This feature was previously only present in the admin console, but is now also available for the account console, login and welcome page. If a user indicates their preference through an operating system setting (e.g. light or dark mode) or a user agent setting, the theme will automatically follow these preferences.

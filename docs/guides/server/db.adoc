...
This command includes the minimum settings needed to connect to the database but it exposes the database password and is not recommended.

The default schema is `keycloak`, but you can change it by using the `db-schema` configuration option.

It is also possible to configure the database when https://www.keycloak.org/server/importExport[importing or exporting Realms] or https://www.keycloak.org/server/bootstrap-admin-recovery[creating new Keycloak admin users / service accounts] (starting with release 26):
----
